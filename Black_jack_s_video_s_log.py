...
class Black_Jack(Koloda):
    ...
    def __init__(self, number, player):  # Ввод количества карт в колоде и числа игроков
        ...
        self.player = player
        self.logging.debug(f'пользователь ввел колличество карт {self.number}')
    ...
    def ochki_card(self):  # Выдача карты с колоды и поиск веса
        ...
        self.search_ochki()
        self.result()
        self.logging.debug(f'полученная карта {self.count}')
    ...
    def search_ochki(self):  # Функция поиска веса карты в значениях
        if self.temp_2 in self.ochki.keys():  # Поиск значения в словаре
            ...
            self.logging.debug(f'очки карты {self.x}')
        else:
            pass
    ...
    def game(self):
        ...
        money_1 = input()
        self.logging.debug(f'пользователь ввел{money_1}')
        print("Ставка компьютера")
        money = input()
        self.logging.debug(f'пользователь ввел{money}')
        if self.player == 1:
            ...
            if self.count > self.count_1 and self.count <= 21:
                ...
                self.video()
        self.logging.debug(f'деньги первого пользователя, его общие очки {money},  {self.count}')
        self.logging.debug(f'деньги второго пользователя, его общие очки  {money_1}, {self.count_1}')
        if self.player == 2:
            ...
            if self.count <= 21 and self.count > self.count_1 and self.count > self.count_2:
                ...
                self.video()
        self.logging.debug(f'Очик первого пользователя {self.count},'
                           f'\nвторого пользователя {self.count_1}, '
                           f'\nтретьего пользователя{self.count_2}')
